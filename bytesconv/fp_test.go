...
// Wrapper around bytesconv.ParseFloat(x, 64).  Handles dddddp+ddd (binary exponent)
// itself, passes the rest on to bytesconv.ParseFloat.
func myatof64(s string) (f float64, ok bool) {
	a := strings.SplitN(s, "p", 2)
	if len(a) == 2 {
		n, err := bytesconv.ParseInt([]byte(a[0]), 10, 64)
		if err != nil {
			return 0, false
		}
		e, err1 := bytesconv.Atoi([]byte(a[1]))
		if err1 != nil {
			println("bad e", a[1])
			return 0, false
		}
	...
	f1, err := bytesconv.ParseFloat([]byte(s), 64)
	if err != nil {
		return 0, false
	}
	return f1, true
...
// Wrapper around bytesconv.ParseFloat(x, 32).  Handles dddddp+ddd (binary exponent)
// itself, passes the rest on to bytesconv.ParseFloat.
func myatof32(s string) (f float32, ok bool) {
	a := strings.SplitN(s, "p", 2)
	if len(a) == 2 {
		n, err := bytesconv.Atoi([]byte(a[0]))
		if err != nil {
			println("bad n", a[0])
			return 0, false
		}
	...
	f64, err1 := bytesconv.ParseFloat([]byte(s), 32)
	f1 := float32(f64)
	if err1 != nil {
		return 0, false
	}
