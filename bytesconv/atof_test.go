...
func init() {
	...
	for i := range atofRandomTests {
		...
		s := []byte(strconv.FormatFloat(x, 'g', -1, 64))
	...
	for i := range benchmarksRandomBits {
		...
		benchmarksRandomBits[i] = []byte(strconv.FormatFloat(x, 'g', -1, 64))
	}

	for i := range benchmarksRandomNormal {
		x := rand.NormFloat64()
		benchmarksRandomNormal[i] = []byte(strconv.FormatFloat(x, 'g', -1, 64))
	}
}
...
func BenchmarkAtof64Big(b *testing.B) {
	s := []byte("123456789123456789123456789")
	for i := 0; i < b.N; i++ {
		ParseFloat(s, 64)
	}
}
...
func BenchmarkAtof32Random(b *testing.B) {
	...
	for i := range float32strings {
		...
		float32strings[i] = []byte(strconv.FormatFloat(float64(math.Float32frombits(n)), 'g', -1, 32))
	}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
