...
func (e *NumError) Error() string {
	return "bytesconv." + e.Func + ": " + "parsing " + strconv.Quote(e.Num) + ": " + e.Err.Error()
}

func numError(fn string, str []byte, err error) *NumError {
	return &NumError{fn, string(str), err}
}
...
func ParseInt(s []byte, base int, bitSize int) (i int64, err error) {
	...
	if err != nil && err.(*NumError).Err != ErrRange {
		return 0, numError(fnParseInt, s0, err.(*NumError).Err)
	}
